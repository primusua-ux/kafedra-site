...
function applyFilters(users: Profile[], sp: Record<string, string | undefined>): Profile[] {
  let result = users;
  ...
  const platoon = sp.platoon?.trim().toLowerCase();
  if (platoon) {
    result = result.filter((u) => u.platoon?.toLowerCase().includes(platoon));
  }

  if (sp.role) {
    result = result.filter((u) => u.role === sp.role);
  }

  if (sp.status) {
    result = result.filter((u) => u.status === sp.status);
  }
...
export default async function AdminPage({
  ...
  const users = applyFilters(allUsers ?? [], sp);

  const isFiltered =
    Boolean(sp.q) || Boolean(sp.platoon) || Boolean(sp.role) || Boolean(sp.status);

  // При активному фільтрі — єдина зведена таблиця
  if (isFiltered) {
    return (
      <>
        <PageHero
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-6">
          ...
          <UsersTable
            title={`Результати пошуку (${users.length})`}
            users={users}
          />
        </section>
      </>
...
function UsersTable({
  ...
  return (
    <div
      className={`border bg-[--color-bg-panel] ${
        highlight
      ...
      {users.length === 0 ? (
        ...
        <div className="divide-y divide-[--color-border]">
          {users.map((u) => (
            <div
              key={u.id}
              className="p-4 sm:p-6 flex flex-col md:flex-row md:items-center gap-3"
            >
              {/* Інформація */}
              <div className="flex-1 min-w-0">
                ...
                {u.role === "student" && u.platoon && (
                  <div className="text-xs text-[--color-text-dim] mt-0.5">
                    Взвод:{" "}
                    <span className="text-[--color-accent]">{u.platoon}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 text-xs shrink-0">
                <span className="uppercase tracking-widest border border-[--color-border-strong] px-2 py-1">
                  {u.role === "student" ? "Студент" : u.role === "teacher" ? "Викладач" : "Адмін"}
                </span>
                <span
                  className={`uppercase tracking-widest border px-2 py-1 ${
                    u.status === "approved"
                ...
                >
                  {u.status === "pending" ? "Очікує" : u.status === "approved" ? "Активний" : "Відхилений"}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 shrink-0">
                <form action={changeRole}>
                  ...
                  <button
                    type="submit"
                    className="ml-1 px-2 py-1.5 text-xs border border-[--color-border-strong] hover:border-[--color-accent]"
                  >
                    Змінити
                  </button>
                </form>
                ...
                {u.status !== "approved" && (
                  <form action={approveUser}>
                    ...
                    <button
                      type="submit"
                      className="inline-flex items-center gap-1 px-3 py-1.5 bg-[--color-accent] text-[--color-bg] text-xs uppercase tracking-wider font-semibold"
                    >
